...
.countrycode_dict = function(myurl='http://s-ecfin-web/directorates/db/u1/R/routines/countrycode_dictionary.csv') {
  intdict = NULL
  ...
  .cgrpdict[['EU27']]= grpsubset('EU',2012)
  ...
  return(list(
    ...
    update = function() {loadDict(TRUE); return(invisible(intdict))},
    groupget = function(x) { 
    ...
    groupset = function(x,value) { 
      ...
      .cgrpdictaliases<<-names(.cgrpdict);
      names(.cgrpdictaliases) <<- ccode(names(.cgrpdict), 'ec','ecb',leaveifNA = TRUE,warn=FALSE)  
    },
    ccodes=function(x) {
...
#'   \item \code{countrycode_dict$update()} update \code{countrycode_dict} to the latest country code conversion table from intranet
